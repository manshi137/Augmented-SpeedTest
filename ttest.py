...
# SequenceNumber,RequestTime,ReplyTime,RequestSourceIP,RequestDestIP,ReplySourceIP,ReplyDestIP,ReplyTTLexpiredIP,TTL,Download/Upload/Idle
ping_reply_map = []
...
# find expectation value of download_diff and upload_diff for each hop
# download_diff_mean = []
# upload_diff_mean = []
# for hop in range(hops):
#     download_mean.append(np.mean(download[hop]))
#     upload_mean.append(np.mean(upload[hop]))
#     idle_mean.append(np.mean(idle[hop]))

# for i in range(hops, 1, -1):
#     download_mean.append(download_mean[i] - download_mean[i-1])
#     upload_mean.append(upload_mean[i] - upload_mean[i-1])
#     idle_mean.append(idle_mean[i] - idle_mean[i-1])
